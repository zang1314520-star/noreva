...
export default function AdminPage() {
  ...
  async function handleDrop(e: React.DragEvent) {
    ...
    try { const url = await uploadImage(file); setNewP((p) => ({ ...p, image: url })); setMsg("Image uploaded!"); }
  ...
  async function addPro() {
    ...
    if (r.ok) { setMsg("Product added!"); setNewP({ name: "", category: "clothing", price: 0, image: "", description: "" }); loadData(); }
    else setMsg("Error"); setTimeout(() => setMsg(""), 3000);
  }

  async function delPro(id) { if (!confirm("Delete?")) return; await fetch("/api/products", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) }); setMsg("Deleted"); loadData(); setTimeout(() => setMsg(""), 3000); }
        ...
        {tab === "products" && (
          <div>
            <div className="bg-white rounded-xl p-6 shadow-sm mb-6">
              ...
              <div className="mb-4">
                <label className="text-sm text-gray-600 block mb-2">Image - Drag and drop or click to upload (auto WebP)</label>
                <div onDragOver={(e) => e.preventDefault()} onDrop={handleDrop} onClick={() => document.getElementById("fileInput")?.click()} className="w-full h-32 border-2 border-dashed rounded-lg flex items-center justify-center cursor-pointer hover:border-[#C9A96E] transition-colors">
                  {newP.image ? <img src={newP.image} alt="" className="h-28 object-contain" /> : <span className="text-gray-400">Drop image here or click</span>}
                </div>
                <input id="fileInput" type="file" accept="image/*" onChange={handleFile} className="hidden" />
              </div>
              <div className="mb-4"><label className="text-sm text-gray-600">Description</label><textarea value={newP.description} onChange={(e) => setNewP({ ...newP, description: e.target.value })} className="w-full p-2 border rounded h-20 resize-none" /></div>
