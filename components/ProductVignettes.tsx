...
function CategoryCard({ category, index }: { category: typeof CATEGORIES[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      onClick={(e) => {
        // On touch devices, first tap opens the overlay instead of navigating
        if (!isHovered && window.matchMedia("(hover: none)").matches) {
          e.preventDefault();
          setIsHovered(true);
        }
      }}
