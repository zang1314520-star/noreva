          ...
          <AnimatePresence>
            {isHovered && (
              <motion.div
                ...
                className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center p-4 overflow-y-auto"
              >
                <motion.h3
                  initial={{ y: 20 }}
                  animate={{ y: 0 }}
                  className="font-display text-xl md:text-2xl text-white mb-3 text-center"
                >
                  {category.nameEn}
                </motion.h3>
                <AnimatePresence mode="wait">
                  {activeSub ? (
                    ...
                    >
                      <button
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); setActiveSub(null); }}
                        className="text-white/70 hover:text-white text-xs underline"
                      >
                        {t("productsBackTo")}
                      </button>
                      <div className="flex flex-wrap justify-center gap-2 px-2">
                        {activeSub.brands.map((brand, i) => (
                          <BrandBubble key={brand} name={brand} index={i} categorySlug={activeSub.slug} />
                        ))}
                      </div>
                      <p className="text-white/60 text-[10px] mt-1">{activeSub.brands.length} brands</p>
                    </motion.div>
                  ) : (
                    <motion.div
                      key="subs"
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="flex flex-wrap justify-center gap-2 max-w-[280px] max-h-[50vh] overflow-y-auto"
                    >
                      {category.subcategories.map((sub) => (
                        <button
                          key={sub.slug}
                          onClick={(e) => { e.preventDefault(); e.stopPropagation(); setActiveSub(sub); }}
                          className="px-3 py-1.5 bg-white/95 rounded-full text-xs text-[#1A1A1A] hover:bg-[#C9A96E] hover:text-white transition-colors whitespace-nowrap"
                        >
                          {sub.nameEn}
                        </button>
                      ))}
  ...
  return (
    <section ref={ref} id="objects" className="py-[clamp(5rem,10vw,9rem)] overflow-hidden bg-white">
      <div className="px-8 md:px-16">
        ...
        >
          <span className="label text-[#8A8A8A] block mb-3">{t("heroSeason")}</span>
          <h2 className="font-display text-[clamp(2rem,3.5vw,2.8rem)] font-light text-[#1A1A1A]">
            {t("collectionsTheObjects")}
          </h2>
        </motion.div>
